...
async function getChipsInternos(base44) {
  ...
  // ✅ RL-1: se já existe refresh em andamento, aguardar (evita N webhooks paralelos chamando filter() simultaneamente)
...
async function handleMessage(dados, payloadBruto, base44) {
  ...
  if (dados.messageId) {
    try {
      ...
      console.warn(`[${VERSION}] ⚠️ WAL.create falhou (não-bloqueante):`, e.message);
    }
  }
