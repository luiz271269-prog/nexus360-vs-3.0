...
export default function Comunicacao() {
  ...
  const { data: contatos = [], isLoading: loadingContatos } = useQuery({
    ...
    queryFn: async () => {
      ...
      try {
        // 🚀 PERF: Buscar APENAS os contatos das threads visíveis (não 1000 contatos)
        const response = await base44.functions.invoke('buscarContatosLivre', {
          contact_ids: contactIdsParaCarregar,
          limit: contactIdsParaCarregar.length
        });

        if (response?.data?.success) {
          return response.data.contatos || []; // ✅ PRESERVA _meta do backend
        }
  ...
  // 🚀 PERF: Enriquecimento desativado no frontend.
  // Backend já enriquece via getOrCreateContactCentralized + processInbound.
  // Re-disparar do frontend causa loop infinito quando Z-API retorna 404.
  ...
  const handleIniciarNovaConversaSemPermissao = React.useCallback(async () => {
    ...
    try {
      ...
      // Criar nova thread canônica atribuída ao usuário atual (campos completos)
      const agora = new Date().toISOString();
      const novaThread = await base44.entities.MessageThread.create({
      ...
      <div className="h-[100dvh] md:h-screen flex flex-col bg-gradient-to-br from-amber-50 via-orange-50/30 to-red-50/20 overflow-hidden">
        ...
        <Tabs value={activeTab} onValueChange={setActiveTab} className="flex-1 flex flex-col overflow-hidden min-h-0">
          ...
          <div className="flex-1 overflow-hidden min-h-0">
            ...
            <TabsContent value="conversas" className="h-full m-0 p-0">
              ...
              <div className="hidden md:flex h-full">
                <div className={`border-r border-slate-200 bg-white flex flex-col overflow-hidden flex-shrink-0 transition-all duration-300 ${sidebarViewMode === 'kanban' ? 'w-full' : 'w-80 xl:w-96'}`}>
                  ...
                  <div className={`flex-1 overflow-hidden transition-opacity duration-200 ${isPendingFilter ? 'opacity-50' : 'opacity-100'}`}>
                    {sidebarViewMode === 'kanban' ? (
                      <ChatSidebarKanban threads={threads} threadAtiva={threadAtiva} onSelecionarThread={handleSelecionarThread} onVoltar={() => setThreadAtiva(null)} usuarioAtual={usuario} integracoes={integracoes} atendentes={atendentes} contatos={contatos} onSelectInternalDestinations={handleInternalSelection} sidebarViewMode={sidebarViewMode} onSidebarViewModeChange={setSidebarViewMode} modoSelecaoMultipla={modoSelecaoMultipla} onModoSelecaoMultiplaChange={setModoSelecaoMultipla} contatosSelecionados={contatosSelecionados} setContatosSelecionados={setContatosSelecionados} onOpenKanbanNaoAtribuidos={() => setMostrarKanbanNaoAtribuidos(true)} onOpenKanbanRequerAtencao={() => setMostrarKanbanRequerAtencao(true)} />
                    ) : (
                ...
                <DesktopChatArea
                  ...
                  selectedCategoria={selectedCategoria} atendentes={atendentes} filterScope={filterScope}
                  selectedIntegrationId={selectedIntegrationId} selectedAttendantId={selectedAttendantId}
                  contatoAtivo={contatoAtivo} showContactInfo={showContactInfo}
                  handleAtualizarContato={handleAtualizarContato} novoContatoTelefone={novoContatoTelefone}
                  contactInitialData={contactInitialData} handleCriarNovoContato={handleCriarNovoContato}
              ...
              <MobileChatArea
                ...
                sidebarViewMode={sidebarViewMode} onSidebarViewModeChange={setSidebarViewMode}
                searchTerm={searchTerm} onSearchChange={setSearchTerm}
                filterScope={filterScope} onFilterScopeChange={(val) => startTransition(() => setFilterScope(val))}
                selectedAttendantId={selectedAttendantId} onSelectedAttendantChange={setSelectedAttendantId}
                atendentes={atendentes} isManager={isManager}
                novoContatoTelefone={novoContatoTelefone} onNovoContatoTelefoneChange={setNovoContatoTelefone} setNovoContatoTelefone={setNovoContatoTelefone}
                integracoes={integracoes} selectedIntegrationId={selectedIntegrationId} onSelectedIntegrationChange={setSelectedIntegrationId}
                selectedCategoria={selectedCategoria} onSelectedCategoriaChange={setSelectedCategoria}
                ...
                mostrarKanbanNaoAtribuidos={mostrarKanbanNaoAtribuidos} setMostrarKanbanNaoAtribuidos={setMostrarKanbanNaoAtribuidos}
                mostrarKanbanRequerAtencao={mostrarKanbanRequerAtencao} setMostrarKanbanRequerAtencao={setMostrarKanbanRequerAtencao}
                ...
                handleVoltarListaMobile={handleVoltarListaMobile}
                handleSelecionarThread={handleSelecionarThread}
                handleInternalSelection={handleInternalSelection}
                handleEnviarMensagemOtimista={handleEnviarMensagemOtimista}
                handleEnviarMensagemInternaOtimista={handleEnviarMensagemInternaOtimista}
            ...
            <ComunicacaoTabsContent
              usuario={usuario}
              todasIntegracoes={todasIntegracoes}
              integracoes={integracoes}
              gotoIntegracoes={gotoIntegracoes}
              ...
              selectedTipoContato={selectedTipoContato}
              selectedTagContato={selectedTagContato}
              threadsFiltradas={threadsFiltradas}
              contatos={contatos}
