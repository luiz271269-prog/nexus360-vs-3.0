...
/**
 * Drawer de Histórico Interno do Orçamento.
 * Permite registrar notas de texto, áudios gravados, e classificar
 * por tipo (nota/ligação/visita/follow-up).
 *
 * Salva em: Orcamento.historico_interno (array)
 */

const TIPOS = [
  { value: 'nota',     label: 'Nota',       icon: MessageSquare, cor: 'bg-amber-500' },
  { value: 'ligacao',  label: 'Ligação',    icon: Phone,         cor: 'bg-blue-500' },
  { value: 'visita',   label: 'Visita',     icon: MapPin,        cor: 'bg-emerald-500' },
  { value: 'followup', label: 'Follow-up',  icon: Bell,          cor: 'bg-purple-500' },
];
...
export default function OrcamentoHistoricoDrawer({ orcamento, isOpen, onClose, onSaved }) {
  ...
  // Carregar histórico + usuário ao abrir
  React.useEffect(() => {
    if (!isOpen) return;
    setHistorico(Array.isArray(orcamento?.historico_interno) ? orcamento.historico_interno : []);
    base44.auth.me().then(setUsuario).catch(() => {});
  }, [isOpen, orcamento?.id]);

  const persistir = async (novoHistorico) => {
    if (!orcamento?.id) {
      toast.error('Salve o orçamento antes de adicionar histórico.');
      return false;
    }
    try {
      await base44.entities.Orcamento.update(orcamento.id, { historico_interno: novoHistorico });
      setHistorico(novoHistorico);
      if (onSaved) onSaved(novoHistorico);
      return true;
    } catch (e) {
      console.error('[Histórico] Erro ao salvar:', e);
  ...
  const handleAdicionarNota = async () => {
    ...
    const novo = [...historico, novaEntrada];
    const ok = await persistir(novo);
    if (ok) {
      setTexto('');
      setTipo('nota');
  ...
  // Quando termina a gravação, faz upload e adiciona no histórico
  React.useEffect(() => {
    ...
    (async () => {
      ...
      try {
        const file = new File([audioBlob], `historico-${Date.now()}.ogg`, {
        ...
        const novaEntrada = {
          ...
          audio_url: file_url,
          audio_duracao_seg: duracaoSegundos || 0,
        };
        const novo = [...historico, novaEntrada];
        const ok = await persistir(novo);
        if (ok) {
          setTexto('');
          toast.success('🎤 Áudio salvo no histórico');
        }
      } catch (e) {
        console.error('[Histórico] Erro upload áudio:', e);
        ...
        <div className="flex items-center justify-between px-4 py-3 bg-gradient-to-r from-amber-500 to-orange-500 text-white flex-shrink-0">
          <div className="flex items-center gap-2 min-w-0">
            ...
            <div className="min-w-0">
              <p className="text-sm font-semibold truncate">Histórico Interno</p>
              <p className="text-[11px] text-white/80 truncate">
                {orcamento?.numero_orcamento ? `#${orcamento.numero_orcamento}` : 'Orçamento'} • {historico.length} {historico.length === 1 ? 'registro' : 'registros'}
              </p>
            </div>
          </div>
          ...
          {historicoOrdenado.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-full text-center px-4 py-12">
              ...
              <p className="text-xs text-slate-500 mt-1 max-w-[260px]">
                Adicione notas, registre ligações, visitas ou follow-ups deste orçamento abaixo.
              </p>
            </div>
          ) : (
            historicoOrdenado.map((entrada) => {
              const tipoInfo = TIPOS.find(t => t.value === entrada.tipo) || { label: 'Áudio', icon: Mic, cor: 'bg-rose-500' };
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center justify-between gap-2 mb-1">
                        <div className="flex items-center gap-1.5 min-w-0">
                          <span className="text-xs font-semibold text-white truncate">{entrada.autor_nome || 'Usuário'}</span>
                          <span className="text-[10px] text-slate-400 flex-shrink-0">• {tipoInfo.label}</span>
                        </div>
                        <span className="text-[10px] text-slate-500 flex-shrink-0">{formatarData(entrada.data)}</span>
                      </div>
                      {entrada.texto && (
                        <p className="text-xs text-slate-200 whitespace-pre-wrap break-words leading-relaxed">{entrada.texto}</p>
                      )}
                      {entrada.audio_url && (
                        <div className="mt-1.5">
                          <AudioPlayer url={entrada.audio_url} />
                        </div>
                      )}
                    </div>
                    <button
