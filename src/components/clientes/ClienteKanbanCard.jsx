...
export default function ClienteKanbanCard({ cliente, score, isDragging, onEdit, onViewDetails, statusGradient }) {
  ...
  return (
    ...
    >
      ...
      <div className="bg-gradient-to-r from-orange-500 to-red-500 px-2 py-1 flex items-center justify-between gap-1.5">
      ...
      <CardContent className="px-2 py-1.5 space-y-1">
        ...
        <div className="flex items-center justify-between gap-1 pt-0.5">
          ...
          >
            <span className="w-6 h-6 rounded-full border border-slate-300 bg-white flex items-center justify-center text-slate-700 transition-all duration-150 group-hover/btn:bg-orange-500 group-hover/btn:text-white group-hover/btn:border-orange-500 group-hover/btn:shadow-md">
              <Edit className="w-3 h-3" />
            </span>
            <span className="text-[9px] text-slate-700">Editar</span>
          </button>
          <AgendarRetornoButton contexto={{ titulo: `Retorno — ${cliente.razao_social || cliente.nome_fantasia || 'Cliente'}`, descricao: 'Acompanhamento comercial do cliente.', contextType: 'Cliente', contextId: cliente.id, clienteId: cliente.id, responsavelId: cliente.usuario_id }} className="h-6 border-violet-200 px-1.5 text-[9px] text-violet-700" />
          <button
            onClick={(e) => { e.stopPropagation(); setHistoricoOpen(true); }}
            title="Histórico interno"
            className="flex items-center gap-1 group/btn"
          >
            <span className="w-6 h-6 rounded-full border border-slate-300 bg-white flex items-center justify-center text-slate-700 transition-all duration-150 group-hover/btn:bg-amber-500 group-hover/btn:text-white group-hover/btn:border-amber-500 group-hover/btn:shadow-md">
          ...
          <button
            ...
            className={`flex items-center gap-1 group/btn ${!temTelefone ? 'cursor-not-allowed opacity-40' : ''}`}
          >
            <span className={`relative w-6 h-6 rounded-full border flex items-center justify-center transition-all duration-150 ${
              temTelefone
