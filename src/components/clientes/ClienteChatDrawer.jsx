 ...
 * dentro do contexto do CRM (card de Cliente), igual ao OrcamentoChatDrawer.
 *
 * Resolução de thread (sem tocar Comunicacao.jsx):
 *   1. telefone canônico do cliente → Contact.telefone_canonico → MessageThread canônica
 *   2. fallback por cliente_id → Contact.cliente_id → MessageThread canônica
 */
export default function ClienteChatDrawer({ cliente, isOpen, onClose }) {
  ...
  // ─── Resolução de thread por telefone/cliente_id ───────────────────────────
  React.useEffect(() => {
    ...
    const resolverThread = async () => {
      ...
      try {
        ...
        // FALLBACK: contato vinculado ao cliente_id
        ...
        console.error('[ClienteChatDrawer] Erro ao resolver thread:', e);
  ...
  const telefoneCliente = cliente?.telefone || cliente?.celular;
