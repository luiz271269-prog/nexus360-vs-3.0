...
const inicial = () => {
  ...
  return { titulo: '', descricao: '', tipo: 'tarefa', prioridade: 'media', data: d.toISOString().slice(0, 10), hora: d.toTimeString().slice(0, 5), semHorario: false, responsavel: '', recorrencia: 'none', lembrete: '60' };
};
...
export default function useNovaTarefa(aberto, contexto, onCriado, onFechar) {
  ...
  useEffect(() => {
    ...
    base44.auth.me().then(async u => {
      setUsuario(u); setForm(f => ({ ...f, responsavel: f.responsavel || u.id, titulo: contexto.titulo || f.titulo, descricao: contexto.descricao || f.descricao }));
      if (u.role === 'admin') setUsuarios(await base44.entities.User.list('-created_date', 100));
    });
