...
import { TASK_TYPES, PRIORITIES, RECURRENCES, REMINDERS } from './agendaTaskUtils';
...
export default function NovaTarefaCampos({ form, alterar, usuario, usuarios }) {
  return <div className="space-y-4">
    ...
    <div className="grid grid-cols-2 gap-3"><CampoSelect label="Recorrência" value={form.recorrencia} onChange={v => alterar('recorrencia', v)} options={RECURRENCES} /><CampoSelect label="Lembrete" value={form.lembrete} onChange={v => alterar('lembrete', v)} options={REMINDERS} /></div>
    {usuario?.role === 'admin' && usuarios.length > 0 && <CampoSelect label="Responsável" value={form.responsavel} onChange={v => alterar('responsavel', v)} options={usuarios.map(u => [u.id, u.full_name || u.email])} />}
