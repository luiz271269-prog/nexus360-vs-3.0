...
import { WhatsAppLogo, InstagramLogo, FacebookLogo, GoToLogo } from './CanalLogos';
...
export default function ChatWindow({
  ...
  const handleEnviarFromInput = React.useCallback(async ({ texto, pastedImage, pastedImagePreview, attachedFile, attachedFileType }) => {
    ...
    // ═══════════════════════════════════════════════════════════════════════
    // 🎯 THREAD DE E-MAIL: resposta agora é feita pelo botão "E-mail" no topo
    // ═══════════════════════════════════════════════════════════════════════
    if (thread?.channel === 'email') {
      toast.info('✉️ Use o botão "E-mail" no topo para responder este e-mail');
      return;
    }
  ...
  const contatoFallback = contatoCompleto || contatoPreCarregado || thread?.contato || null;
  const contatoEfetivo = {
    ...(thread?.contato || {}),
    ...(contatoPreCarregado || {}),
    ...(contatoCompleto || {}),
