...
export default function MessageInput({
  ...
  // ✅ O filtro já é feito no Comunicacao.jsx (whatsapp_permissions.can_view)
  // O can_send é validado em runtime no handleEnviarMensagemOtimista
  ...
  return (
    <form onSubmit={handleEnviar} className="text-gray-950 rounded-lg border-t flex-shrink-0 pb-safe relative bg-[#162446] opacity-70" style={{ paddingBottom: 'max(0.5rem, env(safe-area-inset-bottom))' }}>
      ...
      {integracoes.length > 1 && !modoSelecaoMultipla && thread?.thread_type !== 'team_internal' && thread?.thread_type !== 'sector_group' &&
        <div className="mb-1.5 inline-flex items-center gap-2 max-w-full rounded-full bg-[#494960]/90 backdrop-blur-md border border-white/10 shadow-lg shadow-slate-400/30 px-2">
      ...
      <div className="flex items-end gap-1 md:gap-2 relative py-1 text-[hsl(var(--foreground))] opacity-100 bg-[hsl(var(--background))]">
