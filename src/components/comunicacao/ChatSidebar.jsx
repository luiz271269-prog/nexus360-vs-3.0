        ...
        if (!isThreadInterna) {
          ...
          return (
            <motion.div
              ...
              transition={{ delay: index * 0.05 }}
              onClick={(e) => handleClick(thread, e)} 
              className={`px-3 py-3 flex items-center gap-3 cursor-pointer transition-all border-b border-slate-100 hover:bg-slate-50 ${thread.is_contact_only ? 'bg-slate-50/50' : ''} ${isAtiva ? 'bg-teal-50' : 'bg-white'} ${isSelected ? 'bg-orange-100 border-l-4 border-l-orange-500' : ''}`}
            >
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center justify-between mb-0.5">
                  ...
                  <div className="flex items-center gap-1.5 flex-shrink-0 ml-2">
                    <span className={`text-xs ${hasUnread ? 'text-teal-600' : 'text-slate-400'}`}>
                      {formatarHorario(thread.last_message_at)}
                    </span>
                    {/* Contador WhatsApp Style */}
                    {hasUnread && (
                ...
                <div className="flex items-center justify-between">
                  <p className={`text-sm truncate flex items-center gap-1 flex-1 ${
                    ...
                    {thread.is_contact_only ? (
                      ...
                      <>
                        ...
                        {thread.last_media_type === 'image' && <Image className="w-3.5 h-3.5 flex-shrink-0" />}
                        {thread.last_media_type === 'video' && <Video className="w-3.5 h-3.5 flex-shrink-0" />}
                        {thread.last_media_type === 'audio' && <Mic className="w-3.5 h-3.5 flex-shrink-0" />}
                        {thread.last_media_type === 'document' && <FileText className="w-3.5 h-3.5 flex-shrink-0" />}
                        {thread.last_media_type === 'location' && <MapPin className="w-3.5 h-3.5 flex-shrink-0" />}
                        {thread.last_media_type === 'contact' && <PhoneIcon className="w-3.5 h-3.5 flex-shrink-0" />}
                        <span className="truncate">
                          {(() => {
