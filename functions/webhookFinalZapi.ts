...
async function handleQRCode(dados, base44) {
  ...
  try {
    const integracoes =
      (await base44?.asServiceRole?.entities?.WhatsAppIntegration?.filter?.(
...
async function handleMessage(dados, payloadBruto, base44) {
  ...
  try {
    ...
    console.error(`[${VERSION}] erro_contato`, e?.message || e);
  ...
  try {
    const threads =
      (await base44?.asServiceRole?.entities?.MessageThread?.filter?.(
    ...
    console.error(`[${VERSION}] erro_thread`, e?.message || e);
  ...
  try {
    ...
    console.error(`[${VERSION}] erro_salvar_mensagem`, e?.message || e);
