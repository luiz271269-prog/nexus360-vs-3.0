...
// ===========================================================================
// WEBHOOK WHATSAPP Z-API - v8.1.0 OTIMIZADO
// ===========================================================================
// 1. Filtrar ULTRA-CEDO antes de qualquer operação
// 2. Logs MÍNIMOS - apenas mensagens reais salvas
// 3. Ignorar: status@broadcast, @lid, grupos, fromMe, digitando
// 4. whatsapp_integration_id SEMPRE não metadados
// ===========================================================================
...
function deveIgnorar(payload) {
  ...
  // IGNORAR IMEDIATAMENTE: status@broadcast e JIDs do sistema
  if (phone.includes('status@') || phone.includes('@broadcast') || phone.includes('@lid') || phone.includes('@g.us')) {
    return 'jid_sistema';
  }

  // PERMITIR: QR Code e Conexão
  ...
  // MessageStatusCallback - ignora se for de status@broadcast
  if (tipo.includes('messagestatuscallback')) {
    if (phone.includes('status@') || phone.includes('@broadcast')) {
      return 'status_broadcast';
    }
    return null; // Processar atualizações de status válidos
  }
...
async function handleMessage(dados, payloadBruto, base44) {
  ...
  // Log de auditoria (apenas mensagens reais)
  try {
