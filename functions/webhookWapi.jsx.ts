...
async function handleMessage(dados, payloadBruto, base44) {
  ...
  // 🎯 BUSCAR/CRIAR CONTATO - LÓGICA ANTI-DUPLICATA
  const profilePicUrl = payloadBruto.sender?.profilePicture || payloadBruto.sender?.profilePicThumbObj?.eurl || null;
  let contato;
  try {
    // 1️⃣ BUSCA GLOBAL POR TODAS AS VARIAÇÕES (em uma única consulta)
    const telefoneBase = dados.from.replace(/\D/g, '');
    
    // Gerar TODAS as variações possíveis
    const variacoes = new Set([
      dados.from,                                    // +5548999322400
      dados.from.replace('+', ''),                  // 5548999322400
      telefoneBase,                                  // 5548999322400
      '+' + telefoneBase                            // +5548999322400
    ]);
    
    // Adicionar variações com/sem nono dígito
    if (telefoneBase.startsWith('55') && telefoneBase.length === 13) {
      const semNono = telefoneBase.substring(0, 4) + telefoneBase.substring(5);
      variacoes.add('+' + semNono);
      variacoes.add(semNono);
    }
    
    if (telefoneBase.startsWith('55') && telefoneBase.length === 12) {
      const comNono = telefoneBase.substring(0, 4) + '9' + telefoneBase.substring(4);
      variacoes.add('+' + comNono);
      variacoes.add(comNono);
    }
    
    console.log(`[WAPI] 🔍 Buscando contato com ${variacoes.size} variações:`, Array.from(variacoes));
    
    // 2️⃣ BUSCA PARALELA POR TODAS AS VARIAÇÕES (mais rápido)
    const buscas = Array.from(variacoes).map(tel =>
    ...
    const todosContatos = resultados.flat().filter(Boolean);
    
    // 3️⃣ DEDUPLICAÇÃO: Pegar o mais antigo (created_date)
    let contatosUnicos = [];
    ...
    contatosUnicos.sort((a, b) => new Date(a.created_date) - new Date(b.created_date));

    if (contatosUnicos.length > 0) {
      contato = contatosUnicos[0];
      
      // 🔄 MERGE: Se encontrou múltiplos, marcar os outros como duplicatas
      if (contatosUnicos.length > 1) {
        console.log(`[WAPI] ⚠️ DUPLICATAS ENCONTRADAS: ${contatosUnicos.length} contatos. Master: ${contato.id}`);
        
        for (let i = 1; i < contatosUnicos.length; i++) {
          const duplicata = contatosUnicos[i];
          try {
            await base44.asServiceRole.entities.Contact.update(duplicata.id, {
              observacoes: `DUPLICATA - Merged para ${contato.id} em ${new Date().toISOString()}`
            });
            console.log(`[WAPI] 🔀 Duplicata marcada: ${duplicata.id}`);
          } catch {}
        }
      }
      
      // Atualizar contato master
      const update = { ultima_interacao: new Date().toISOString() };
      if (dados.pushName && (!contato.nome || contato.nome === dados.from || contato.nome.length < dados.pushName.length)) {
      ...
      console.log(`[WAPI] 👤 Contato existente (ID: ${contato.id}): ${contato.nome}`);
      
    } else {
      // 4️⃣ CRIAR NOVO (nenhum encontrado)
      ...
      console.log(`[WAPI] 👤 NOVO contato criado (ID: ${contato.id}): ${contato.nome}`);
    }
  } catch (e) {
