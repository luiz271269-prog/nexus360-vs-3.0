...
/**
 * INICIAR FLUXO OAUTH - Instagram (Meta Business)
 * Gera URL de autorização para conectar Instagram Business Account
 */
...
Deno.serve(async (req) => {
  try {
    ...
    // Scopes necessários para Instagram Direct Messages
    const scopes = [
      'instagram_basic',
    ...
    console.error('[INSTAGRAM OAUTH INIT] Erro:', error);
    return Response.json({ 
      error: error.message 
    }, { status: 500 });
  }
